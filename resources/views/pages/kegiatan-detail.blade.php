    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-lg-4">
                ...
                <a href="{{ route('explore.detail', $rec->slug) }}" class="text-decoration-none d-block mb-3">
...
<section class="container py-5">
    <div class="row">
        <div class="col-lg-8 mx-auto">
            <div class="card mb-4">
                @if($event->media)
                    <img src="{{ asset('storage/' . $event->media) }}" class="card-img-top" alt="{{ $event->judul }}">
                @endif
                <div class="card-body">
                    <h2 class="mb-3">{{ $event->judul }}</h2>
                    <p>{{ $event->desc }}</p>
                </div>
            </div>

            <h4 class="mb-3">Event Lainnya</h4>
            <div class="row">
                @foreach ($recommended as $rec)
                <div class="col-md-6">
                    <div class="card mb-3">
                        @if($rec->media)
                        <img src="{{ asset('storage/' . $rec->media) }}" class="card-img-top" alt="{{ $rec->judul }}">
                        @endif
                        <div class="card-body">
                            <h5>{{ $rec->judul }}</h5>
                            <p>{{ Str::limit($rec->desc, 70) }}</p>
                            <a href="{{ route('events.detail', ['id' => $e->id]) }}" class="btn btn-primary btn-sm">Lihat Detail</a>
                        </div>
                    </div>
                </div>
                @endforeach
