...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <form action="{{ route('news.update', $news) }}" method="POST"
        ...
        <div class="mb-4">
            ...
            <textarea name="link" id="link" rows="3"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('link') border-red-500 @enderror">{{ old('link', $news->link) }}</textarea>
            @error('link')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="desc" id="desc" rows="10"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('desc') border-red-500 @enderror">{{ old('desc', $news->desc) }}</textarea>
            @error('desc')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @if($news->media)
                <div class="mb-3">
                    <img src="{{ $news->image_url }}" alt="{{ $news->title }}" class="img-thumbnail"
                        style="max-width: 300px;">
                    <p class="text-muted small mt-1">Current media (upload new to replace)</p>
                </div>
            @endif
